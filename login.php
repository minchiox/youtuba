...
if(isset($_POST["submit"])){
  ...
  $result = mysqli_query($conn, "SELECT * FROM tb_user WHERE email ='$username' ");
  $row = mysqli_fetch_assoc($result);
  if(mysqli_num_rows($result) > 0){
     if($password == $row["password"]){
       //$_SESSION["id"]= $row["id"];
       $_SESSION['fullname']= $row["nome_cognome"]; //farei diventare nome e cognome in solo nome
...
<head>
  ...
  <meta charset="UTF-8">
  <title>Youtuba - Login Page</title>
      ...
      <div class="col-sm-5 authfy-panel-left">
        <div class="brand-col">
          <div class="headline">
            ...
            <p>Questa è la sezioni di log-in, accedi per nuove importanti lezioni sulla tuba</p>
          </div>
        </div>
      </div>
        ...
        <div class="authfy-login">
          ...
          <div class="authfy-panel panel-login text-center active">
            <div class="authfy-heading">
              <h3 class="auth-title">Accedi al tuo account</h3>
              <p>Non hai un'account? <a class="lnk-toggler" data-panel=".panel-signup" href="registration.php">Iscriviti gratis!</a></p>
            </div>
            <div class="row">
              <div class="col-xs-12 col-sm-12">
                <form name="loginForm" class="loginForm" action="#" method="POST">
                 ...
                 <!-- ./remember-row -->
                  <div class="form-group">
                    <button class="btn btn-lg btn-primary btn-block" name="submit" type="submit">Accedi!</a>
                  </div>
                </form>
              </div>
            </div>
